package course.m5_functional_domain_modeling

// .___________. __    __   _______         ___      .__   __.      _______.____    __    ____  _______ .______          _______.
//|           ||  |  |  | |   ____|       /   \     |  \ |  |     /       |\   \  /  \  /   / |   ____||   _  \        /       |
//`---|  |----`|  |__|  | |  |__         /  ^  \    |   \|  |    |   (----` \   \/    \/   /  |  |__   |  |_)  |      |   (----`
//   |  |     |   __   | |   __|       /  /_\  \   |  . `  |     \   \      \            /   |   __|  |      /        \   \
//   |  |     |  |  |  | |  |____     /  _____  \  |  |\   | .----)   |      \    /\    /    |  |____ |  |\  \----.----)   |
//   |__|     |__|  |__| |_______|   /__/     \__\ |__| \__| |_______/        \__/  \__/     |_______|| _| `._____|_______/

/** HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  *
  * HEY! Go away! These are the answers :P
  */
object Answers {

  val allBooleanValues   = Set(true, false)
  val booleanCardinality = 2

  val theThingCardinality = 1
  val rpsCardinality      = 3
  val rpsThumbCardinality = 5

  val booleanBoxCardinality    = 4
  val rpsRoundBoxCardinality   = 18
  val booleanBoxBoxCardinality = 8

  val optionBooleanCardinality       = 3
  val optionRPSCardinality           = 4
  val optionOptionBooleanCardinality = 4
  val optionBoxOptionRPSCardinality  = 9
}
