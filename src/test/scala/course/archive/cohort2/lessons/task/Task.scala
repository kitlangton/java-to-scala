...
//      case result                     => throw new Error(s"THIS SHOULD NEVER HAPPEN $result")
//    }
//  }
//
//  import Task._
...
//      fiberA <- self.fork
//      fiberB <- that.fork
//      a      <- fiberA.join.foldCause(cause => fiberB.interrupt *> Task.Fail(cause), a => Task(a))
//      b      <- fiberB.join.foldCause(cause => fiberA.interrupt *> Task.Fail(cause), b => Task(b))
//    } yield (a, b)
//}
//
//object Task {
...
//    }
//
//  def collectAll[A](as: List[Task[A]]): Task[List[A]] =
//    as.foldRight(Task.succeed(Nil): Task[List[A]]) { (a, acc) =>
//      a.zip(acc).map { case (b, bs) => b :: bs }
//    }
//
//  def collectAllPar[A](as: List[Task[A]]): Task[List[A]] =
//    as.foldRight(Task.succeed(Nil): Task[List[A]]) { (a, acc) =>
//      a.zipPar(acc).map { case (b, bs) => b :: bs }
//    }
//
