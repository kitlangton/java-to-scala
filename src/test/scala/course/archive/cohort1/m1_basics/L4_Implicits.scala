...
/**   - For any given type there should be a single canonical instance
  *     - associative way of combining lists (++)
  *     - if you cannot define it in the companion object, rethink it.
  *     - extension methods
  *     - Prove a type is a subtype: evidence!
  *
  *   - CON
  *     - using implicits instead of passing parameters around (execution
  *       context)
  *   - PROnciple
  *
  *   - type classes
...
/** Implicits are very unique and powerful feature of Scala. They also can be
  ...
  */
object ImplicitParameters extends Exercise {
...
//   val missingExplicitArguments = explicitMethod // <- Uncomment this line
  ...
  implicit val cool: String = "implicit"
  ...
  finalParameterList(fortyTwo, cool) // <- Uncomment this line
...
//     s"$w, $x, $y, $z"
  ...
  def validateConfig(implicit config: Config): Boolean =
    config.port > 1000

  def createConnection(name: String)(implicit config: Config): Unit =
    validateConfig

  def setUpDatabase(name: String, timeout: Int)(implicit config: Config): Unit = {
    validateConfig
    createConnection(name)
  }

  implicit val config: Config = Config(8080, "localhost")

  setUpDatabase("Cool Database", 10_000)
  ...
  val implicitlyTest = test("implicitly") {
    ...
    lazy val summoned: Butler = implicitly // <-- Summon jeeves by using the `implicitly` method
  ...
  val tacitlyTest = test("tacitly") {
    ...
    def tacitly[A](implicit a: A): A = a // <-- Finish this method
...
//  implicit val noIAmSpartacus: Spartacus = Spartacus()
  ...
  /** ✏ EXERCISE
    ...
    */
  @implicitNotFound("Uh oh. Oh no! Where's the light switch? I can't find the light switch!")
...
object ImplicitResolution_InScope {
  ...
  /** This MacGuffin is defined in the scope, and thus trivially available to
    ...
    */
  implicit val m1: MacGuffin = MacGuffin(100)
...
trait ImplicitExtension {
  implicit val extensionGuffin: MacGuffin = MacGuffin(100)
}

object MacGuffin {
  implicit val companionGuffin: MacGuffin = MacGuffin(100)
}

object ImplicitResolution_Precedence { // 2

//  import treasure.macGuffin // explicit import
//  import riches._ // wildcard

  def main(args: Array[String]): Unit =
//    implicit val macGuffin: MacGuffin = MacGuffin(0) // in scope
...
object ImplicitResolution_Companion {
  ...
  // 1. vals/defs in scope
  // 2. brought in via traits
  // 3. explicit imports
  // 4. wildcard imports
  // 5. companion object of the implicit type definitions
  // 5. companion object of type variables of the implicit type definitions
...
object ImplicitConversions extends App {
  ...
  final case class MyString(string: String)
...
object MoreAdvancedBiz {
  ...
  object NotBoolean {
    implicit def notBoolean[A]: NotBoolean[A] = new NotBoolean[A] {}

    @implicitAmbiguous("THIS CANNOT WORK ON BOOLEANS FOR SOME GODFORSAKEN REASON")
    implicit def notBoolean2: NotBoolean[Boolean] = new NotBoolean[Boolean] {}
    implicit def notBoolean3: NotBoolean[Boolean] = new NotBoolean[Boolean] {}
  }

  final case class Box[A](value: A) {
    def not(implicit ev: A Is Boolean): Box[Boolean] = Box(!ev(value))

    def anythingButBoolean(implicit ev: NotBoolean[A]): Int = 12
  }

  private val value: Box[Boolean] = Box(true)
  value.not
//  value.anythingButBoolean
  Box(12).anythingButBoolean
